...
await scenario('live shape: Gold Dust (GD) + Atriq (AT), with stock',
  ...
  { shops: 'AT:Atriq | GD:Goldust | OF:Office | RG:Rose Gold', keepsIdOf: 1 });
...
await scenario('empty database', null, { shops: 'AT:Atriq | GD:Goldust | OF:Office | RG:Rose Gold' });
...
await scenario('a third shop exists and is left alone',
  ...
  { shops: 'AT:Atriq | GD:Goldust | KK:Attrick | OF:Office | RG:Rose Gold', keepsIdOf: 1 });
...
const mapped = readCodes({
  ...
  SHOP_CODE_OF: 'office-secret',
  SHOP_CODE_ZZ: 'no-such-shop',
  STAFF_CODE: 'legacy-secret',
  SHOP_CODE_TOOLONG: 'ignored',
  SHOP_CODE_AT_BLANK: '',
});
check('    all four shops map',
  mapped.RG === 'rose-secret' && mapped.AT === 'atriq-secret'
  && mapped.GD === 'goldust-secret' && mapped.OF === 'office-secret', JSON.stringify(mapped));
...
const secrets = [mapped.RG, mapped.AT, mapped.GD, mapped.OF];
check('    each shop has its own key', new Set(secrets).size === 4, secrets.join(' / '));
