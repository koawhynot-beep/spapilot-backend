...
// Sheet history: a huge sale on a 15th (2026-09-15 is a Tuesday).
await db.query(`INSERT INTO stock_movements (item_id, shop_id, type, qty_change, occurred_at, unit_price, note) VALUES
  (3,1,'sale',-500,'${at('2026-09-15')}',700000,$1)`, [SALES_IMPORT_NOTE]);
...
const weekdays = async () => (await db.query(
  `SELECT EXTRACT(ISODOW FROM ${LOCAL_AT_SQL})::int AS dow,
    ...
    WHERE ${SALE_TYPES_SQL} AND COALESCE(m.note,'') <> $1
    GROUP BY 1 ORDER BY 1`, [SALES_IMPORT_NOTE]
...
check('the sheet history IS counted for the garment ranking — the day does not matter there',
  top[0].sku === 'S-3' && top[0].pieces === 502, `${top[0].sku} ${top[0].pieces}`);
...
check('the sheet note is excluded from the weekday query',
  /COALESCE\(m\.note,''\) <> \$/.test(pep), 'the sheet sales are counted in the weekdays');
